...
module.exports = (function(){
  ...
  socketServer.on('job-success', (id, jobData, result) => {
    logger.info(`job success: ${jobData}, ${result}`)
    socketLogger(`[${id}]job success`);
    socketLogger(`[${id}]add idle worker`);
    workerPool.addIdle(id);
    socketServer.broadcast('success-notice', jobData, result)
  })
  ...
  socketServer.on('job-failure', (id, jobData) => {
    ...
    socketLogger(`[${id}]add idle worker`);
    workerPool.addIdle(id);
    socketServer.broadcast('failure-notice', jobData);
  })
